...
const bListPkmn = pokémon.filter( p => p.name[0] === "B" ); // gets all pokemon names start with the letter "B"
console.log(bListPkmn);

const pkmnIds = pokémon.map( p => p.id ); // gets all pokemon id's
console.log(pkmnIds);

//pokemon objects id divisible by 3
...
const twoPlusTypes = pokémon.filter (p => p.types.length > 1);
console.log(twoPlusTypes);
...
const normalPkmn = pokémon.filter(p => p.types[0] === "normal").length;
